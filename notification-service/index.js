...
app.post('/v1/notifications/email', async (req, res) => {
  ...
  // Support both 'text' and 'message' fields for flexibility
  const emailBody = text || message;
  
  if (!to || !subject || !emailBody) {
    return res.status(400).json({ 
      error: 'Invalid request payload - to, subject, and text/message are required',
      received: { to, subject, text, message }
    });
  }

  try {
    // For demo purposes, just log the email instead of actually sending
    console.log(`[${req.correlationId}] Email would be sent:`);
    console.log(`To: ${to}`);
